...
try {
  ...
  const legacyResult = await legacyPi.events.get("before_agent_start")({ systemPrompt: "base prompt" }, legacyCtx);
  assert.equal(
    legacyResult,
    undefined,
  ...
  const ownedResult = await ownerPi.events.get("before_agent_start")({ systemPrompt: "base prompt" }, ownerCtx);
  assert.match(
  ...
  const formerOwnerResult = await ownerPi.events.get("before_agent_start")({ systemPrompt: "base prompt" }, ownerCtx);
  assert.equal(
    formerOwnerResult,
    undefined,
  ...
  const claimantResult = await claimantPi.events.get("before_agent_start")(
