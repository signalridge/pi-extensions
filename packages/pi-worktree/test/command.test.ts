...
test("remove deletes only a confirmed clean linked worktree and verifies deregistration", async () => {
  ...
  (mock.rawPi as typeof mock.rawPi & { exec: ExecFunction }).exec = async (_command, args) => {
    calls.push(args);
    if (args[0] === "worktree" && args[1] === "list") {
      const targetPath = quarantinePath ?? linked;
      return result(
        porcelain([
          { path: main, branch: "main" },
          ...(!removed
            ? [
                {
                  path: targetPath,
                  branch: "feature",
                  prunableReason: quarantinePath ? "gitdir file points to non-existent location" : undefined,
                },
              ]
            : []),
        ]),
    ...
    if (args[0] === "worktree" && args[1] === "prune") {
      if (args[2] === "--dry-run")
        return result("Removing worktrees/repo-feature: gitdir file points to non-existent location\n");
      removed = true;
      return result();
    }
    return result();
  ...
  try {
    await mock.commands.get("worktree")?.handler("", context.ctx);
    const moveCall = calls.find((args) => args[0] === "worktree" && args[1] === "move");
    const pruneCall = calls.find((args) => args[0] === "worktree" && args[1] === "prune" && args[2] !== "--dry-run");
    assert.ok(moveCall);
    assert.ok(pruneCall);
    assert.equal(moveCall[2], linked);
    assert.deepEqual(pruneCall.slice(0, 3), ["worktree", "prune", "--expire"]);
...
test("remove explicitly confirms and discards reflog-only recovery history", async () => {
  ...
  const main = join(root, "repo");
  const linked = join(root, "repo-feature");
  const administrative = join(root, "administrative", "repo-feature");
  mkdirSync(main);
  mkdirSync(linked);
...
test("remove revalidates worktree identity after waiting for the mutation lock", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-worktree-remove-lock-race-"));
  const main = join(root, "repo");
  const linked = join(root, "repo-feature");
  const administrative = join(root, "administrative", "repo-feature");
  mkdirSync(main);
  ...
  let resolveBeforeRemoval!: () => void;
  const beforeRemoval = new Promise<void>((resolve) => {
    resolveBeforeRemoval = resolve;
  });
  ...
  (mock.rawPi as typeof mock.rawPi & { exec: ExecFunction }).exec = async (_command, args) => {
    if (args[0] === "worktree" && args[1] === "list") {
      listCalls += 1;
      if (listCalls === 2) resolveBeforeRemoval();
      const replacement = listCalls >= 3;
      return result(
        porcelain([
          { path: main, branch: "main" },
          {
            path: linked,
            branch: replacement ? "replacement" : "feature",
            head: replacement ? oid.replace(/^0/, "1") : oid,
          },
        ]),
      );
    }
    if (args[0] === "rev-parse" && args.includes("--git-dir")) return result(`${administrative}\n`);
    if (args[0] === "rev-parse") return result(`${main}\n`);
    if (args[0] === "status" || args[0] === "ls-files" || args[0] === "submodule") return result();
    if (args.includes("for-each-ref")) return result();
    if (args[0] === "worktree" && args[1] === "move") {
      moveCalls += 1;
      emulateGitWorktreeMove(args);
  ...
  const heldLock = withWorktreeMutationLock(main, async () => lockGate);
  try {
    ...
    assert.match(context.notifications.at(-1)?.message ?? "", /changed identity while waiting for removal/i);
  } finally {
    releaseLock();
    await heldLock;
    rmSync(root, { recursive: true, force: true });
...
test("prune confirmation names administrative candidates omitted from Git preview", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-worktree-prune-omitted-candidate-"));
  const main = join(root, "repo");
  const admin = join(main, ".git", "worktrees", "hidden");
  mkdirSync(admin, { recursive: true });
  writeFileSync(join(admin, "HEAD"), "ref: refs/heads/feature\n");
  ...
  let actualPruneCalls = 0;
  (mock.rawPi as typeof mock.rawPi & { exec: ExecFunction }).exec = async (_command, args) => {
    if (args[0] === "worktree" && args[1] === "list") return result(porcelain([{ path: main, branch: "main" }]));
    if (args[0] === "rev-parse" && args.includes("--git-common-dir")) return result(".git\n");
    if (args[0] === "rev-parse" && args.includes("--show-toplevel")) return result(`${main}\n`);
    if (args[0]?.startsWith("--git-dir=") && args[1] === "diff") return result();
    if (args[0] === "worktree" && args[1] === "prune") {
      if (args[2] === "--dry-run") return result("Removing worktrees/other: missing gitdir\n");
      actualPruneCalls += 1;
    }
    return result();
  };
  ...
  try {
    await mock.commands.get("worktree")?.handler("", context.ctx);
    assert.match(confirmation, /hidden/);
    assert.match(confirmation, /administrative metadata records selected for deletion/i);
    assert.equal(actualPruneCalls, 0);
    assert.equal(readFileSync(join(admin, "HEAD"), "utf8"), "ref: refs/heads/feature\n");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
const movedOid = "89abcdef0123456789abcdef0123456789abcdef";

interface AddHarnessOptions {
  branchExists?: boolean;
  /** Runs while the confirmation dialog is awaited, i.e. between the preview and the recheck. */
  onConfirm?: (state: AddHarnessState, paths: AddHarnessPaths) => void;
  /** HEAD reported by porcelain for the created record; defaults to the tip used by the add. */
  createdHead?: string;
}

interface AddHarnessPaths {
  main: string;
  linked: string;
  other: string;
}

interface AddHarnessState {
  branchExists: boolean;
  tip: string;
  records: Array<{ path: string; branch?: string; head?: string }>;
}

/**
 * Drive the real /worktree Add flow against a mutable Git mock. Drift is injected from `onConfirm`
 * because `ctx.ui.confirm` is awaited, so a mutation there lands between the approved preview and
 * the post-confirmation recheck — exactly where a concurrent Git process would land.
 */
function createAddHarness(options: AddHarnessOptions = {}) {
  const root = mkdtempSync(join(tmpdir(), "pi-worktree-add-provenance-"));
  const main = join(root, "repo");
  const linked = join(root, "repo-feature");
  const other = join(root, "repo-other");
  mkdirSync(main);
  const state: AddHarnessState = {
    branchExists: options.branchExists ?? false,
    tip: oid,
    records: [{ path: main, branch: "main" }],
  };
  const calls: string[][] = [];
  let mutations = 0;
  let previewMessage = "";
  ...
  (mock.rawPi as typeof mock.rawPi & { exec: ExecFunction }).exec = async (_command, args) => {
    calls.push([...args]);
    if (args[0] === "worktree" && args[1] === "list") return result(porcelain(state.records));
    if (args[0] === "rev-parse" && args[1] === "--show-toplevel") return result(`${main}\n`);
    if (args[0] === "check-ref-format") return result("feature\n");
    if (args[0] === "show-ref") return result("", state.branchExists ? 0 : 1);
    if (args[0] === "symbolic-ref") return result("main\n");
    if (args[0] === "rev-parse" && args[1] === "--verify") return result(`${state.tip}\n`);
    if (args[0] === "worktree" && args[1] === "add") {
      mutations += 1;
      state.records.push({
        path: linked,
        branch: "feature",
        head: options.createdHead ?? state.tip,
      });
      return result();
    }
    return result();
  ...
  const inputs = state.branchExists ? ["feature", linked] : ["feature", "", linked];
  let confirmCount = 0;
  const context = createMockContext({
    cwd: main,
    hasUI: true,
    ...
    confirm: async (_title: string, message: string) => {
      confirmCount += 1;
      if (confirmCount > 1) return false; // decline "Switch Pi workspace?"
      previewMessage = message;
      options.onConfirm?.(state, { main, linked, other });
      return true;
    },
  });
  ...
  return {
    ...
    mutations: () => mutations,
    addArgs: () => calls.find((args) => args[0] === "worktree" && args[1] === "add"),
    run: () => mock.commands.get("worktree")?.handler("", context.ctx),
    cleanup: () => rmSync(root, { recursive: true, force: true }),
  };
}

test("add preview names the branch, base ref, full base commit, and path", async () => {
  const harness = createAddHarness({ branchExists: true });
  try {
    await harness.run();
    assert.match(harness.preview(), /Branch:\s+feature/);
    assert.match(harness.preview(), /Base:\s+refs\/heads\/feature/);
    assert.match(harness.preview(), new RegExp(`Base commit:\\s+${oid}`));
    assert.ok(harness.preview().includes(harness.linked), harness.preview());
...
test("add attaches an existing branch, resolves provenance from refs/heads, and verifies the base", async () => {
  const harness = createAddHarness({ branchExists: true });
  try {
    await harness.run();
    assert.deepEqual(harness.addArgs(), ["worktree", "add", harness.linked, "feature"]);
    const resolved = harness.calls
      .filter((args) => args[0] === "rev-parse" && args[1] === "--verify")
      .map((args) => args[3]);
    // A same-named tag would win over the branch if the bare name were resolved.
    assert.ok(resolved.includes("refs/heads/feature^{commit}"), resolved.join(","));
    assert.equal(resolved.includes("feature^{commit}"), false, resolved.join(","));
    assert.match(harness.context.notifications.at(-1)?.message ?? "", new RegExp(oid));
  } finally {
    harness.cleanup();
  }
});

test("add refuses when the new-branch start point moves after confirmation", async () => {
  ...
  try {
    ...
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /moved from .* to .* after confirmation/i);
  } finally {
    harness.cleanup();
  }
});

test("add refuses when an attached branch tip moves after confirmation", async () => {
  const harness = createAddHarness({
    branchExists: true,
    onConfirm: (state) => {
      state.tip = movedOid;
  ...
  try {
    await harness.run();
    assert.equal(harness.mutations(), 0);
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /refs\/heads\/feature moved from/i);
  } finally {
    harness.cleanup();
  }
});

test("add refuses when the branch becomes checked out elsewhere after confirmation", async () => {
  const harness = createAddHarness({
    onConfirm: (state, paths) => {
      state.records.push({ path: paths.other, branch: "feature" });
    },
  });
  try {
    await harness.run();
    assert.equal(harness.mutations(), 0);
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /was checked out at/i);
  } finally {
    harness.cleanup();
  }
});

test("add refuses when the target path is registered after confirmation", async () => {
  const harness = createAddHarness({
    onConfirm: (state, paths) => {
      state.records.push({ path: paths.linked, branch: "other" });
    },
  });
  try {
    await harness.run();
    assert.equal(harness.mutations(), 0);
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /became a registered worktree/i);
  } finally {
    harness.cleanup();
  }
});

test("add refuses when another process creates the branch after confirmation", async () => {
  const harness = createAddHarness({
    onConfirm: (state) => {
  ...
  try {
    await harness.run();
    assert.equal(harness.mutations(), 0);
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /created by another process/i);
  } finally {
    harness.cleanup();
  }
});

test("add refuses when an attached branch is deleted after confirmation", async () => {
  const harness = createAddHarness({
    branchExists: true,
    onConfirm: (state) => {
  ...
  try {
    await harness.run();
    assert.equal(harness.mutations(), 0);
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /was deleted after confirmation/i);
  } finally {
    harness.cleanup();
  }
});

test("add reports a retained worktree when the created head is not the approved base", async () => {
  const harness = createAddHarness({ branchExists: true, createdHead: movedOid });
  try {
    await harness.run();
    assert.equal(harness.mutations(), 1);
    assert.match(harness.context.notifications.at(-1)?.message ?? "", /retained at .* verification failed/i);
  } finally {
    harness.cleanup();
  }
});
