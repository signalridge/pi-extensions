...
Fresh implementation only auto-submits when the destination's provider/model ID matches the authenticated source model. A different or missing destination model leaves the complete handoff in the replacement editor with both identities in a warning: select or confirm the intended model, then submit manually. Changing the source model during authentication cancels the attempt before replacement; confirm the model and retry `/plan`.
