...
test("fresh Plan mode uses configured default tools and restores previous tools", async () => {
  await withAgentDir(async (agentDir) => {
    ...
    assert.equal(await hook?.({ toolName: "custom", input: { arbitrary: { nested: true } } }, context.ctx), undefined);
...
test("missing and empty default tool settings remain distinct", async () => {
  await withAgentDir(async (agentDir) => {
    ...
    assert.deepEqual(missing.rawPi.getActiveTools(), ["bash", "grep", "read", ...REQUIRED_PLAN_TOOLS]);
...
test("restored session tool selections override configured defaults", async () => {
  await withAgentDir(async (agentDir) => {
    await writeFile(join(agentDir, "pi-plan-mode.json"), JSON.stringify({ defaultPlanTools: ["bash", "custom"] }));
    const allTools = [builtinTool("read"), builtinTool("bash"), builtinTool("grep"), extensionTool("custom")];
...
test("settings reload resets removed or invalid configured defaults", async () => {
  await withAgentDir(async (agentDir) => {
    ...
    const allTools = [builtinTool("read"), builtinTool("bash"), builtinTool("grep"), builtinTool("write")];
    const mock = createMockPi({ activeTools: ["write"], allTools });
    planMode(mock.pi);
    const context = createMockContext();
...
test("configured names follow effective sources without dynamic auto-activation", async () => {
  await withAgentDir(async (agentDir) => {
    await writeFile(join(agentDir, "pi-plan-mode.json"), JSON.stringify({ defaultPlanTools: ["read", "late"] }));
...
test("the tool selector persists a session override and shutdown restores prior tools", async () => {
  await withAgentDir(async (agentDir) => {
    await writeFile(join(agentDir, "pi-plan-mode.json"), JSON.stringify({ defaultPlanTools: ["bash", "custom"] }));
    const allTools = [builtinTool("read"), builtinTool("bash"), builtinTool("write"), extensionTool("custom")];
    const mock = createMockPi({ activeTools: ["write"], allTools });
    planMode(mock.pi);
    ...
    assert.deepEqual(mock.rawPi.getActiveTools(), ["bash", "read", "custom", ...REQUIRED_PLAN_TOOLS]);
    ...
    assert.deepEqual(resumed.rawPi.getActiveTools(), ["bash", "read", "custom", ...REQUIRED_PLAN_TOOLS]);
  });
});
...
test("the Plan-mode tool selector searches metadata and toggles the stable tool name", async () => {
  await withAgentDir(async (agentDir) => {
    await writeFile(join(agentDir, "pi-plan-mode.json"), JSON.stringify({ defaultPlanTools: ["bash"] }));
    const allTools = [
      builtinTool("read"),
      builtinTool("bash"),
      builtinTool("write"),
