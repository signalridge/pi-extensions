...
test("issue 302: re-entered Plan Mode hides the previous implementation handoff", async () => {
  ...
  assert.deepEqual(mock.rawPi.getActiveTools(), ["bash", "read", "plan_mode_question", "plan_mode_complete"]);
  ...
  assert.deepEqual(activeContext.messages, [implementationMessages[0], implementationMessages[2], activeMessages[3]]);
  assert.doesNotMatch(
