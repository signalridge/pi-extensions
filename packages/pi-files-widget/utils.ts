...
export function hasCommand(cmd: string, options: CommandLookupOptions = {}): boolean {
  ...
  const searchDirectories = platform === "win32" ? [options.cwd ?? process.cwd(), ...pathEntries] : pathEntries;
  const extensions =
    platform === "win32" && !extname(cmd)
  ...
  for (const rawDirectory of searchDirectories) {
    const directory =
      platform === "win32" && rawDirectory.startsWith('"') && rawDirectory.endsWith('"')
