...
export const WORKFLOW_CAPABILITIES: readonly CapabilityEntry[] = [
  {
    ...
    options: [
      ...
      { name: "model", kind: "string", optional: true, default: "unsupported by spawn-managed (refused)" },
      { name: "tier", kind: "small|medium|large", optional: true, default: "pi-subagents workflow tier" },
      { name: "isolation", kind: '"worktree"', optional: true, default: "agent-owned only" },
    ...
    constraints: [
      ...
      "per-call `model` is refused (not silently substituted); route via tier",
    ],
  },
  {
    name: "parallel",
