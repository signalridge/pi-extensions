...
// `environmentDefaultConfig()` reads PI_STATUSLINE_PRESET at load time, so an
// exported preset in the developer's shell would silently change the defaults
// these cases assert. Unset it for the suite and restore it afterwards.
const previousSuitePreset = process.env.PI_STATUSLINE_PRESET;
delete process.env.PI_STATUSLINE_PRESET;
afterAll(() => {
  if (previousSuitePreset === undefined) delete process.env.PI_STATUSLINE_PRESET;
  else process.env.PI_STATUSLINE_PRESET = previousSuitePreset;
});

test("initial JSON exposes the restrained Signalridge defaults without materializing a palette", () => {
  assert.equal(DEFAULT_STATUSLINE_CONFIG.palettePreset, "classic");
  assert.deepEqual(DEFAULT_STATUSLINE_CONFIG.palette, {});
  assert.equal(DEFAULT_STATUSLINE_CONFIG.density, "compact");
  assert.equal(DEFAULT_STATUSLINE_CONFIG.separator, "dot");
  assert.deepEqual(DEFAULT_STATUSLINE_CONFIG.segments, ["model", "thinking", "cwd", "branch", "tools", "context"]);
...
test("normalization supports partial icon-only settings and structured overrides", () => {
  ...
  assert.deepEqual(normalized.config.segmentText.cache, {
    prefix: "Cache: ",
    suffix: "",
  });
  assert.deepEqual(normalized.config.segmentText.turn, {
    prefix: "#",
    suffix: " turns",
  });
  ...
  const iconOnly = normalizeStatuslineConfig({
    extensionStatusIcons: { goal: "◎" },
  });
  assert.equal(iconOnly.config.palettePreset, "classic");
...
test("explicit palette preset takes precedence while preserving custom colors", () => {
  ...
  const defaultedCustom = normalizeStatuslineConfig({
...
test("normalization falls back by field and reports unknown, duplicate, and invalid values", () => {
  ...
  assert.equal(normalized.config.palettePreset, "classic");
...
test("all named palettes, separators, empty segments, and environment independence are accepted", () => {
  ...
  try {
    ...
    assert.equal(normalizeStatuslineConfig({}).config.palettePreset, "classic");
  } finally {
...
test("classic environment selection resolves the Signalridge default without writing settings", () => {
  const root = mkdtempSync(join(tmpdir(), "pi-statusline-classic-default-"));
  const previous = process.env.PI_STATUSLINE_PRESET;
  process.env.PI_STATUSLINE_PRESET = "classic";
  try {
    const loaded = loadStatuslineSettingsForAgent(root);
    ...
    assert.throws(
      () =>
        saveStatuslineSettingsDocument(
          path,
          JSON.stringify({
            segments: ["model", "line_break", "line_break", "cwd"],
          }),
        ),
