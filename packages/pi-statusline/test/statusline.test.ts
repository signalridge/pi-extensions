...
test("statusline renders an incremental usage snapshot without rescanning history", async () => {
  const mock = createMockPi();
  ...
  const entries: unknown[] = [assistant];
  let scans = 0;
  statusline(mock.pi);
  ...
  try {
    const scansAfterRebuild = scans;
    footer.render(200);
    footer.render(200);
    footer.render(200);
    assert.equal(scans, scansAfterRebuild, "repeated renders must not walk session history");
    ...
    await emit(mock.events, "message_end", { type: "message_end", message: updatedAssistant }, context.ctx);
    footer.render(200);
    assert.equal(scans, scansAfterRebuild);
    ...
    await emit(mock.events, "session_compact", { type: "session_compact" }, context.ctx);
    assert.ok(scans > scansAfterRebuild, "compaction rebuilds the accumulator");
    footer.render(200);
  } finally {
    footer.dispose();
  }
...
test("a replaced session cannot invoke the statusline command", async () => {
  const mock = createMockPi();
  statusline(mock.pi);
  ...
  await mock.commands.get("statusline")?.handler("help", previous.ctx);
...
test("statusline renders max thinking in the Tokyo Night footer", async () => {
  ...
  try {
    assert.match(footer.render(200)[0] ?? "", /think max/);
  } finally {
    footer.dispose();
  }
});
...
test("formatToolActivity shows only active or streaming work", () => {
  ...
  assert.equal(formatToolActivity(runtime({ activeTools: new Map([["read", 2]]), isStreaming: false })), "read×2");
...
test("extension status helpers strip prefixes, icons, and simplify text", () => {
  ...
  // No space between the badge and the word it labels: the same badge, so it
  // is peeled off the same way.
  assert.deepEqual(splitExtensionStatusIcon("🔌MCP: 6 servers"), {
    icon: "🔌",
    text: "MCP: 6 servers",
  });
  assert.deepEqual(splitExtensionStatusIcon("🔌 ✅ MCP: ready"), {
    icon: "🔌",
    text: "MCP: ready",
  });
  assert.deepEqual(splitExtensionStatusIcon("plain status"), {
    text: "plain status",
  });
  assert.equal(stripExtensionStatusPrefix("firecrawl", "firecrawl: ready"), "ready");
  assert.equal(simplifyExtensionStatusText("ready, missing (details)"), "ready missing");
...
test("statusline settings load extension icon overrides", () => {
  ...
  writeFileSync(
    settingsPath,
    JSON.stringify({
      extensionStatusIcons: { goal: "", "custom-state": "☕", bad: 1 },
    }),
  );
  const configured = readStatuslineSettings(settingsPath);
  assert.equal(configured.extensionStatusIcons.goal, "");
  assert.equal(configured.extensionStatusIcons["custom-state"], "☕");
...
test("extension status icons use config, leading emoji, defaults, and fallback", () => {
  ...
  const config = (extensionStatusIcons: Record<string, string>) => ({
  ...
  assert.equal(
    ...
    "github-pr PR #123 checks passing",
  );
  assert.equal(
    formatExtensionStatus(
      "github-pr",
    ...
    "github-pr PR #123: checks pending (12) changes requested 45 comments",
  );
  assert.equal(
    formatExtensionStatus("custom-state", "☕ display", theme, config({ "custom-state": "🍵" })),
    "🍵 display",
  );
  assert.equal(formatExtensionStatus("custom-state", "☕ display", theme, config({})), "custom-state display");
  assert.equal(formatExtensionStatus("goal", "active", theme, config({ goal: "" })), "goal active");
  assert.equal(formatExtensionStatus("unknown", "running", theme, config({})), "unknown running");
});

test("extension status text renders as plain text when the extension styles or decorates it", () => {
  const theme = { fg: (_color: string, text: string) => text } as never;
  const config = (extensionStatusIcons: Record<string, string> = {}) => ({
    extensionStatusIcons,
  });

  // pi-mcp-adapter styles its whole status before handing it over, which used
  // to hide the leading emoji from the icon splitter and leak the plug badge.
  assert.equal(
    formatExtensionStatus("mcp", "\x1b[38;5;213m🔌 MCP: 6 servers enabled\x1b[39m", theme, config()),
    "mcp 6 servers enabled",
  );
  assert.equal(formatExtensionStatus("mcp", "🔌MCP: 6 servers enabled", theme, config()), "mcp 6 servers enabled");
  assert.equal(formatExtensionStatus("build", "passing ✅ 3 targets 🚀", theme, config()), "build passing 3 targets");
  // A configured icon is the user's own choice and still renders.
  assert.equal(formatExtensionStatus("mcp", "🔌 MCP: ready", theme, config({ mcp: "M" })), "M ready");
  // An all-emoji status would otherwise collapse to an empty segment.
  assert.equal(formatExtensionStatus("herdr", "🟢", theme, config()), "herdr");
  assert.equal(stripStatusEmoji("🔌 MCP: 6 servers"), "MCP: 6 servers");
  assert.equal(stripStatusEmoji("🟢", "herdr"), "herdr");

  // Only colors go: github-pr publishes its PR number as an OSC 8 hyperlink,
  // and the footer keeps it clickable.
  const link = "\x1b]8;;https://github.com/o/r/pull/123\x07#123\x1b]8;;\x07";
  assert.equal(
    formatExtensionStatus("github-pr", `\x1b[32mPR ${link} passing\x1b[39m`, theme, config()),
    `github-pr PR ${link} passing`,
  );
});
...
test("extension status icons match arbitrary exact keys and explicit namespace wildcards", () => {
  ...
  // An empty configured icon means "no badge", so the key labels it instead.
  assert.equal(formatExtensionStatus("foo:worker", "running", theme, config({ "foo:*": "" })), "foo:worker running");
  for (const key of ["foo", "foobar", "foo/server"]) {
    assert.equal(formatExtensionStatus(key, "running", theme, config({ "foo:*": "🧪" })), `${key} running`);
  }
});
...
test("extension status icons use installed package id aliases without fuzzy matching", () => {
  ...
  // "foobar" is not the "foo" status base, so the alias misses and the key
  // labels the row instead of a badge.
  assert.equal(
...
test("extension status icon aliases preserve exact-key precedence and skip ambiguous packages", () => {
  ...
  // Two packages claim the same status base, so the alias is dropped as
  // ambiguous and the key labels the row instead of a badge.
  assert.equal(
    formatExtensionStatus("foo:server", "running", theme, config({ "@first/pi-foo": "1️⃣" }), aliases),
    "foo:server running",
  );

  // An exact key set to "" suppresses the badge the alias would have supplied;
  // the key still labels the row.
...
test("statusline compact formatting helpers", () => {
  ...
  assert.equal(npmPackageName("npm:@signalridge/pi-goal@0.4.1"), "@signalridge/pi-goal");
