...
test("information profiles expose curated segment sets in deterministic order", () => {
  assert.deepEqual(INFORMATION_PROFILES.minimal, ["model", "thinking", "branch", "context"]);
  assert.deepEqual(INFORMATION_PROFILES.balanced, ["model", "thinking", "cwd", "branch", "tools", "context"]);
