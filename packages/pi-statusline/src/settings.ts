...
// Extension statuses are text-first by default. A user may still opt into a
// state-specific badge in settings, but inactive metadata should not add icons
// to the persistent footer.
...
export const DEFAULT_STATUSLINE_CONFIG: StatuslineConfig = {
  ...
  // The text profile does not use hard-coded ANSI backgrounds. Keep this
  // palette empty so it is also a safe starting point for custom settings.
  palette: {},
  density: "compact",
  separator: "dot",
...
export function loadStatuslineSettingsForAgent(agentDir = getAgentDir()): LoadedStatuslineSettings {
  ...
  return isMissingStatuslineSettings(legacy)
    ? builtInSettings(canonicalPath, [], environmentDefaultConfig())
...
function builtInSettings(
  settingsPath: string,
  diagnostics: StatuslineConfigDiagnostic[] = [],
  config: StatuslineConfig = createDefaultConfig(),
): LoadedStatuslineSettings {
  return {
    config,
    source: "built-in",
...
function environmentDefaultConfig(): StatuslineConfig {
  const config = createDefaultConfig();
  const requested = process.env.PI_STATUSLINE_PRESET?.trim().toLowerCase();
  if (requested === "tokyo-night") {
    config.palettePreset = "tokyo-night";
    config.palette = segmentPaletteForPreset("tokyo-night");
    config.separator = "none";
  }
  // `classic` and an unset/unknown value intentionally use the calm
  // Signalridge text profile. Environment defaults apply only when neither
  // settings filename exists; a valid canonical or legacy document wins.
  return config;
}
