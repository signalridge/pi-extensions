...
async function choosePalettePreset(ctx: ExtensionCommandContext, options: StatuslineCommandOptions) {
  if (ctx.mode !== "tui") {
...
function segmentControlHints(width: number, moveMode: boolean, selected: SegmentName | undefined): string[] {
  if (moveMode) {
    return width < 30
      ? [`Move mode: ${selected ?? "segment"}`, "↑↓ move segment", "Enter/Space finish", "Esc leave move mode"]
      : [`Move mode — ${selected ?? "segment"}`, "↑↓ move · Enter/Space finish · Esc leave move mode"];
  }
  return width < 30
    ? ["↑↓ navigate", "Enter/Space toggle", "M move · B line break", "Alt+↑/↓ quick move", "Esc close"]
    : [
