...
export class GoalCommandController {
  ...
  async startGoal(
    ...
    if (existingGoal) {
      ...
      if (
        ...
        notifyTerminal(ctx.ui, "The goal queue changed while confirmation was open. Try again.", "warning");
        return;
      }
    }
  ...
  async addGoal(objective: string, tokenBudget: number | undefined, ctx: StatusContext) {
    ...
    this.runtime.persistGoal(this.runtime.activeGoal);
    notifyTerminal(ctx.ui, `Goal added at position ${this.runtime.queuedGoals.length + 1}: ${objective}`, "info");
  }
  ...
  dropLastGoal(ctx: StatusContext) {
    ...
    if (!result.goal) {
      this.runtime.clearActiveGoal(ctx);
      notifyTerminal(ctx.ui, `Goal dropped: ${result.removed?.text ?? currentGoal.text}`, "warning");
      return;
    }
  ...
  async dispatchPendingQueueActionIfSettled(ctx: StatusContext) {
    ...
    if (!sent && this.runtime.activeGoal?.id === activatedGoal.id) {
      ...
      notifyTerminal(ctx.ui, `Next goal paused after prompt delivery failed: ${activatedGoal.text}`, "warning");
      return false;
    }
    notifyTerminal(
      ctx.ui,
