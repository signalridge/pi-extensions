...
test("extension input cannot claim a pending Pi retry under priority", async () => {
  ...
  await harness.mock.events.get("input")?.[0]?.({ source: "extension", text: "unrelated extension work" }, harness.ctx);

  const unrelatedStart = await beforeStart?.({ prompt: "unrelated extension work", systemPrompt: "base" }, harness.ctx);
...
test("a pending busy skip consumes an accepted owned prompt before advancement", async () => {
  ...
  const result = await harness.mock.events.get("input")?.[0]?.({ source: "extension", text: ownedPrompt }, harness.ctx);
  assert.deepEqual(result, { action: "handled" });
  assert.equal(aborts, 0);
});
...
test("disabled settings freeze retained queues without losing state", async () => {
  ...
  const harness = await createHarness({ sessionManager: { getBranch: () => branch, getEntries: () => branch } }, false);
