...
export function buildGraphModel(hourly: Map<number, Map<HourlyKey, HourlyCell>>, options: GraphOptions): GraphModel {
  ...
  if (merged.length > 0) {
    ...
    for (const [groupKey, groupTotal] of merged) {
      ...
      if (!groupPoints) continue;
      for (let i = 0; i < bucketCount; i++) points[i] += groupPoints[i] ?? 0;
      total += groupTotal;
    }
    series.push({
...
export function renderChart(model: GraphModel, options: ChartRenderOptions): string[] {
  ...
  const plot = (seriesIndex: number, points: number[], firstIdx: number, lastIdx: number) => {
    ...
    for (let i = firstIdx; i <= lastIdx; i++) {
      ...
      const y = Math.round((1 - (points[i] ?? 0) / yMax) * (dotH - 1));
      if (prevX >= 0) {
  ...
  const setDot = (x: number, y: number, seriesIndex: number) => {
    ...
    const masks = cellMasks[row];
    const owners = cellOwner[row];
    if (!masks || !owners) return;
    masks[col] = (masks[col] ?? 0) | (DOT_BITS[x % 2]?.[y % 4] ?? 0);
    owners[col] = seriesIndex;
  };
  ...
  for (let row = 0; row < plotHeight; row++) {
    ...
    for (let col = 0; col < plotWidth; col++) {
      const mask = cellMasks[row]?.[col] ?? 0;
      const owner = mask === 0 ? -2 : (cellOwner[row]?.[col] ?? -2);
      if (owner !== runOwner) {
        flush();
        runOwner = owner;
      }
