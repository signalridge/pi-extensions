...
import { makeHourlyKey } from "../data.ts";
import { buildGraphModel, MAX_GROUP_SERIES, OTHER_SERIES_KEY, renderChart, TOTAL_SERIES_KEY } from "../graph.ts";
...
test("buildGraphModel groups by thinking level with unknown fallback and by model", () => {
  ...
  assert.deepEqual(byModel.series.map((s) => s.label).sort(), ["Total", "m1", "m2"].sort());
});
