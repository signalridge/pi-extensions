...
- Serialize cache updates with an inter-process lock, merge entries from the current cache, and atomically replace unique temporary files so concurrent writers do not lose usage data.
- Deduplicate stable entry IDs only across explicit `parentSession` ancestry, including repeated conflicting fingerprints; independent session files with the same ID and accounting are no longer silently dropped.

### Internal
- Cache format is **v7**. The first `/usage` open after upgrading rebuilds the cache so parent-session lineage proves copied-branch relationships; independent sessions that reuse an entry ID remain counted.
