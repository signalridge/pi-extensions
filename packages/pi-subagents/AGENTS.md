...
- Behavior changes use a root Changeset as the source of pending release notes; do not add duplicate notes to `CHANGELOG.md` before `changeset version` generates the release section.
...
- Attribute external contributions with the issue/PR number and contributor name when applicable.

## Releasing

Published package releases use the repository root's Changesets workflow:

- For a behavior change, run `bun run changeset` from the repository root and select
  `@signalridge/pi-subagents` with the appropriate SemVer bump; the Changeset summary is the canonical pending release note, not a duplicate `CHANGELOG.md` entry.
- Merge the generated Version Packages PR before publication. Changesets versions packages
  independently and publishes only packages named by their changesets.
- Run `bun run check` before merging code changes. Release versioning, npm provenance, and tags are
  handled by `.github/workflows/publish-packages.yml`.

Do not run `npm version` or `npm publish` manually. New package-name bootstrap and partial-release
recovery use the root commands documented in [`docs/releasing.md`](../../docs/releasing.md), never
normal Changesets publication.
