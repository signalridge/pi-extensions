...
import { atomicCreateFile, atomicReplaceFile, buildNewAgentFile, disableInContent, enableInContent, findAgentFile, isEmptyStub, personalAgentsDir, projectAgentsDir, removeFileIfUnchanged, serializeAgentFile, validateAgentFileContent } from "./agent-file-toggle.js";
import {
  AgentManager,
...
import { safeTerminalText, sanitizeDisplayText, truncateCodePoints } from "./ui/safe-text.js";
import { showSchedulesMenu } from "./ui/schedule-menu.js";
import { selectItem } from "./ui/select-item.js";
import { getAgentStatusColor, getAgentStatusLabel, getAgentStatusMark } from "./ui/status-label.js";
...
export function summarizeAgentRuns(agents: readonly Pick<AgentRecord, "status">[]): string {
  const counts = {
    running: 0,
    queued: 0,
    completed: 0,
  ...
  for (const agent of agents) {
    switch (agent.status) {
      ...
      case "steered": counts.wrappedUp++; break;
      case "stopped": counts.stopped++; break;
      case "aborted": counts.aborted++; break;
      case "error": counts.failed++; break;
    }
  }
  return `Agent runs (${agents.length}) · ${counts.running} running · ${counts.queued} queued · ` +
...
export function renderRunningAgentStatus(
  ...
  container.addChild(new Text(mark + theme.fg("accent", "running") + stats, 0, 0));
...
interface RootRuntime {
  onSessionStart: (event: { reason?: string }, ctx: ExtensionContext) => Promise<void>;
}
...
export default function (pi: ExtensionAPI): void {
  ...
  if (childSession) {
    ...
    const unsubscribeChildContext = registerChildContextHandler(pi.events, true);
  ...
  const unsubscribeRootContextRegistration = registerChildContextHandler(pi.events, false);
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (!rootRuntime) {
      try {
        ...
        const startupAgents = loadCustomAgents(ctx.cwd, startupSettings.strictAgentFiles === true);
        rootRuntime = activateRootRuntime(pi, startupSettings, startupAgents, ctx.cwd, unsubscribeRootContext);
      } catch (error: unknown) {
...
function activateRootRuntime(
    ...
    (message, { expanded }, theme) => {
      ...
      function renderOne(d: NotificationDetails): string {
        ...
        if (expanded) {
          ...
          const preview = truncateCodePoints(sanitizeDisplayText(resultPreview.split("\n")[0] ?? ""), 80, "");
  ...
  const groupJoin = new GroupJoinManager(
    (records, partial) => {
      for (const r of records) { agentActivity.delete(r.id); fleet.onAgentFinished(r.id); }

      const groupKey = `group:${records.map(r => r.id).join(",")}`;
      scheduleNudge(groupKey, () => {
        // Re-check at send time
        const unconsumed = records.filter(r => !r.resultConsumed);
        if (unconsumed.length === 0) { fleet.update(); return; }
  ...
  const boundedPersistedText = (value: string | undefined): string | undefined => {
  ...
  function managedRecoveryEventData(tombstone: ManagedSpawnTombstone) {
    ...
    return {
      ...
      status: tombstone.state === "interrupted" ? "interrupted" : failed ? "error" : "completed",
      result: terminal?.result,
      error: terminal?.error,
      durationMs: terminal ? Math.max(0, terminal.completedAt - tombstone.createdAt) : 0,
      ...(terminal?.outputFile ? { outputFile: terminal.outputFile } : {}),
      ...(terminal?.tokenCount ? { tokens: { total: terminal.tokenCount } } : {}),
  ...
  function persistRecoveredManaged(tombstone: ManagedSpawnTombstone): void {
    ...
    if (tombstone.state === "completed") pi.events.emit("subagents:completed", eventData);
  ...
  const manager = new AgentManager((record) => {
    ...
    append: (tombstone) => pi.appendEntry(MANAGED_SPAWN_ENTRY_TYPE, tombstone),
  });
  ...
  const MANAGER_TAKEOVER_LOCK_KEY = Symbol.for("pi-subagents:manager-takeover-lock");
  const withManagerTakeoverLock = async <T>(operation: () => Promise<T>): Promise<T> => {
    const globalRegistry = globalThis as any;
    const predecessor = (globalRegistry[MANAGER_TAKEOVER_LOCK_KEY] as Promise<void> | undefined)
      ?? Promise.resolve();
    let release!: () => void;
  ...
  const claimRpcRegistry = async (ctx: ExtensionContext, allowLiveTakeover: boolean): Promise<void> => {
    await withManagerTakeoverLock(async () => {
      ...
      const contenders = [...new Set([currentOwner, activeOwner])]
        .filter((entry) => entry !== undefined && entry !== registryEntry);
      ...
      for (const contender of contenders) {
        ...
        if (contenderIsLive !== false && contenderSessionId && contenderSessionId !== incomingSessionId) return;
  ...
  async function initializeSession(event: { reason?: string }, ctx: ExtensionContext): Promise<void> {
    sessionCwd = ctx.cwd;
    boundSessionId = ctx.sessionManager?.getSessionId?.();
    currentCtx = ctx;
    await claimRpcRegistry(ctx, event.reason === "reload");
    const sessionEntries = typeof ctx.sessionManager.getBranch === "function" ? ctx.sessionManager.getBranch() : [];
    ...
    if (ownsRpcRegistry && !rpcHandle) {
      rpcHandle = registerRpcHandlers({
        ...
        manager: {
          spawn: spawnTopLevel,
          spawnManaged: (piRef, ctxRef, request) => spawnManagedWithUi(
          ...
          abort: (id) => {
            ...
            return !!record && !record.owner && !record.parentAgentId && manager.abort(id);
          },
          abortOwned: (id, owner) => manager.abortOwned(id, owner),
          quiesceOwned: (runId, agentIds, timeoutMs, owners) => manager.quiesceOwned(runId, agentIds, timeoutMs, owners),
        },
      });
    ...
    if (ownsRpcRegistry && isSchedulingEnabled() && !scheduler.isActive()) startScheduler(ctx);
  }
  ...
  pi.on("session_tree", (_event, ctx) => {
    ...
    const recovered = manager.restoreManagedSpawns(entries as unknown as ManagedSpawnEntryLike[], { dropActive: true });
    manager.clearCompleted(true);
    for (const tombstone of recovered) persistRecoveredManaged(tombstone);
  ...
  const shutdownRuntime = (): Promise<void> => {
    runtimeShutdown ??= (async () => {
      ...
      if (ownsRpcRegistry && (globalThis as any)[MANAGER_ACTIVE_KEY] === registryEntry) {
        delete (globalThis as any)[MANAGER_ACTIVE_KEY];
      }
      if (ownsRpcRegistry && (globalThis as any)[RPC_OWNER_KEY] === registryEntry) {
  ...
  function spawnManagedWithUi(
    ...
    const outputTranscript = getAgentConfig(dispatch.type)?.outputTranscript ?? getOutputTranscriptDefault();
    let id = "";
    const initializeRecord = (spawnedId: string): void => {
      ...
      if (record && outputTranscript) {
        record.outputFile = createOutputFilePath(ctxRef.cwd, id, ctxRef.sessionManager.getSessionId());
        writeInitialEntry(record.outputFile, id, normalizedRequest.prompt, ctxRef.cwd);
      }
      fleet.ensureTimer();
      fleet.update();
    };
    ...
    const managedCallbacks = {
      ...
      onSessionCreated: (session: AgentSession) => {
        ...
        if (record?.outputFile) {
          record.outputCleanup = streamToOutputFile(session, record.outputFile, id, ctxRef.cwd);
        }
      },
      onCompaction: () => {
        fleet.update();
    ...
    const managedResult = manager.spawnManaged(piRef, ctxRef, normalizedRequest, managedPolicy, managedCallbacks);
    ...
    if (!id) {
      ...
      if (record?.status === "queued" || record?.status === "running") initializeRecord(managedResult.id);
    }
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  pi.registerTool(defineTool({
    ...
    renderResult(result, { expanded, isPartial }, theme, renderContext) {
      ...
      const text = safeTerminalText(result.content[0]?.type === "text" ? result.content[0].text : "");
      ...
      if (details.status === "queued") {
        ...
        return new Text(theme.fg("dim", `${getAgentStatusMark("queued")} queued · background${id}`), 0, 0);
      }
      ...
      if (details.status === "background") {
        const id = details.agentId ? ` · ID ${details.agentId}` : "";
        return new Text(theme.fg("dim", `${getAgentStatusMark("running")} running · background${id}`), 0, 0);
      }
      ...
      if (details.status === "completed" || details.status === "steered") {
        ...
        const s = stats(details);
        let line = theme.fg(statusColor, `${getAgentStatusMark(details.status)} ${statusText}`);
      ...
      if (isError) {
        line += "\n" + theme.fg("error", `  Error: ${sanitizeDisplayText(details.error ?? "unknown")}`);
      } else {
  ...
  pi.registerTool(defineTool({
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, _ctx) => {
      ...
      if (!record.session) {
        ...
        if (!record.detached) pi.events.emit("subagents:steered", { id: record.id, message: params.message });
      ...
      (tui, theme, keybindings, done) => {
        return new ConversationViewer(tui, session, record, activity, theme, done, () => {
          if (manager.abort(record.id)) {
            ctx.ui.notify(`Stopped "${sanitizeDisplayText(record.description)}".`, "info");
          }
        }, keybindings, (message: string) => manager.steer(record.id, message), () => manager.getRecord(record.id));
      },
      {
        overlay: true,
  ...
  function activeAgentFile(name: string, cfg: AgentConfig): ReturnType<typeof findAgentFile> {
    return cfg.sourcePath ? findAgentFile(name, sessionCwd, cfg.sourcePath) : undefined;
  }
  ...
  async function showAgentDetail(ctx: ExtensionCommandContext, name: string) {
    ...
    if ((choice === "Edit" || choice === "Delete" || choice === "Reset to default") && !file) {
      ctx.ui.notify(`Cannot ${choice.toLowerCase()} ${name}: its active definition is no longer available. Reload and try again.`, "error");
      return;
    }
    ...
    if (choice === "Edit" && file) {
      ...
      if (edited !== undefined && edited !== content) {
        try {
          ...
          ctx.ui.notify(`Cannot update ${file.path}: ${error instanceof Error ? error.message : String(error)}`, "error");
          return;
        }
        reloadCustomAgents();
        const active = getAgentConfig(name);
        if (active?.sourcePath !== file.path) {
          ctx.ui.notify(`Updated ${file.path}, but it is no longer the active definition after reload.`, "error");
        } else {
      ...
      if (file) {
        ...
        if (confirmed) {
          try {
            ...
            ctx.ui.notify(`Cannot delete ${file.path}: ${error instanceof Error ? error.message : String(error)}`, "error");
            return;
          }
          reloadCustomAgents();
      ...
      if (confirmed) {
        try {
          ...
          ctx.ui.notify(`Cannot reset ${file.path}: ${error instanceof Error ? error.message : String(error)}`, "error");
          return;
        }
        reloadCustomAgents();
        const active = getAgentConfig(name);
        if (active?.isDefault !== true) {
          ctx.ui.notify(`Deleted ${file.path}, but another override is still active for ${name}.`, "warning");
        } else {
  ...
  async function ejectAgent(ctx: ExtensionCommandContext, name: string, cfg: AgentConfig) {
    ...
    try {
      ...
      ctx.ui.notify(`Cannot eject ${name}: ${error instanceof Error ? error.message : String(error)}`, "error");
      return;
    }
    reloadCustomAgents();
    const active = getAgentConfig(name);
    if (active?.sourcePath !== targetPath) {
      ctx.ui.notify(`Ejected ${name} to ${targetPath}, but it did not become the active definition after reload.`, "error");
      return;
    }
  ...
  async function disableAgent(ctx: ExtensionCommandContext, name: string) {
    ...
    if (cfg?.sourcePath && !file) {
      ctx.ui.notify(`Cannot disable ${name}: its active definition ${cfg.sourcePath} is no longer available. Reload and try again.`, "error");
      return;
    }
    if (file) {
      const content = readFileSync(file.path, "utf-8");
      ...
      if (outcome === "invalid") {
        ctx.ui.notify(`Cannot disable ${name}: modified frontmatter is invalid${error ? ` (${error})` : ""}.`, "error");
        return;
      }
      try {
        await atomicReplaceFile(file.path, updated, content);
      } catch (writeError: unknown) {
        ctx.ui.notify(`Cannot disable ${name}: ${writeError instanceof Error ? writeError.message : String(writeError)}`, "error");
        return;
      }
      reloadCustomAgents();
      const active = getAgentConfig(name);
      if (active?.sourcePath !== file.path || active.enabled !== false) {
        ctx.ui.notify(`Cannot disable ${name}: ${file.path} is no longer the active definition after reload.`, "error");
        return;
      }
    ...
    if (!cfg) {
      ctx.ui.notify(`Cannot disable ${name}: no active agent configuration.`, "error");
      return;
    }
    ...
    const existing = existsSync(targetPath) ? readFileSync(targetPath, "utf-8") : undefined;
    if (existing !== undefined) {
      const overwrite = await ctx.ui.confirm("Overwrite", `${targetPath} already exists. Overwrite it with a disable stub?`);
      if (!overwrite) return;
    }
    try {
      if (existing === undefined) await atomicCreateFile(targetPath, "---\nenabled: false\n---\n");
      else await atomicReplaceFile(targetPath, "---\nenabled: false\n---\n", existing);
    } catch (error: unknown) {
      ctx.ui.notify(`Cannot disable ${name}: ${error instanceof Error ? error.message : String(error)}`, "error");
      return;
    }
    reloadCustomAgents();
    const active = getAgentConfig(name);
    if (active?.sourcePath !== targetPath || active.enabled !== false) {
      ctx.ui.notify(`Cannot disable ${name}: ${targetPath} did not become the active definition after reload.`, "error");
      return;
    }
  ...
  async function enableAgent(ctx: ExtensionCommandContext, name: string) {
    ...
    if (error) {
      ctx.ui.notify(`Cannot enable ${name}: modified frontmatter is invalid (${error}).`, "error");
      return;
    }
    ...
    if (isEmptyStub(updated)) {
      try {
        ...
        ctx.ui.notify(`Cannot enable ${name}: ${writeError instanceof Error ? writeError.message : String(writeError)}`, "error");
        return;
      }
      reloadCustomAgents();
      const active = getAgentConfig(name);
      if (active?.enabled === false) {
        ctx.ui.notify(`Cannot enable ${name}: ${file.path} is still disabled after reload.`, "error");
        return;
      }
      ...
      const active = getAgentConfig(name);
      if (active?.sourcePath !== file.path || active.enabled === false) {
        ctx.ui.notify(`Cannot enable ${name}: ${file.path} is no longer the active definition after reload.`, "error");
        return;
      }
  ...
  async function showGenerateWizard(ctx: ExtensionCommandContext, targetDir: string) {
    ...
    if (!name) {
      ctx.ui.notify("Agent name must be 1-64 letters, numbers, dots, underscores, or hyphens, starting with a letter or number.", "warning");
      return;
    }
...
Do not wrap the response in a markdown code fence. Return only the file contents.`;
    ...
    try {
      ...
      ctx.ui.notify(`Generation failed: ${error instanceof Error ? error.message : String(error)}`, "warning");
      return;
    }

    if (record.status !== "completed") {
      ctx.ui.notify(`Generation failed: ${record.error ?? `agent ended with status ${record.status}`}`, "warning");
      return;
    }
    ...
    try {
      ...
      ctx.ui.notify(`Generated agent definition is malformed: ${error instanceof Error ? error.message : String(error)}`, "warning");
      return;
    }
    ...
    try {
      ...
      ctx.ui.notify(`Cannot create ${targetPath}: ${error instanceof Error ? error.message : String(error)}`, "error");
      return;
    }
    ...
    function buildItems(): SettingItem[] {
      ...
      return [
        ...
        },
        {
          ...
          description: "Fail startup on unreadable or unparseable agent files instead of skipping them",
        ...
        },
        {
          id: "fleetView",
          label: "Fleet view",
          description: "Claude Code-style main and subagents list below the editor (down or left to navigate, Enter to view)",
