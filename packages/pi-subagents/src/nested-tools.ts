...
export interface NestedAgentManager {
  ...
  /** Internal live lookup used only by the scoped manager runtime. */
...
function lookupRecord(context: NestedToolContext, id: string): AgentRecord | undefined {
  return context.manager.getRecordMutable(id);
}
...
export function createNestedSubagentTools(context: NestedToolContext): ToolDefinition[] {
  ...
  const agentTool = defineTool({
    ...
    execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
      if (params.resume) {
        const existing = lookupRecord(context, params.resume);
        ...
        if (signal?.aborted) {
          return textResult("Resume aborted.", true);
        }
      ...
      const rootSessionId = lookupRecord(context, context.parentAgentId)?.rootSessionId;
      ...
      const options: NestedSpawnOptions = {
        ...
        onAssistantUsage: (usage) => {
          for (let id: string | undefined = context.parentAgentId; id !== undefined; ) {
            const ancestor = lookupRecord(context, id);
            if (!ancestor) break;
      ...
      const attachTranscript = (id: string): void => {
        ...
        const rec = lookupRecord(context, id);
        if (!rec) return;
      ...
      options.onSessionCreated = (session) => {
        const rec = childId === undefined ? undefined : lookupRecord(context, childId);
