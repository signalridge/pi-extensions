...
describe("agent type registry", () => {
  ...
  describe("default agents", () => {
    ...
    it("pins no model, leaving the choice to the tier catalogue", () => {
      // A built-in that named a vendor would be the same end-run around the
      // catalogue that agent frontmatter is no longer allowed to make — and it
      // would name a model the machine may not have. With no tier configured
      // this inherits the parent's, which is the documented fallback.
      for (const name of ["Explore", "Plan", "general-purpose"]) {
        expect(getAgentConfig(name)?.model).toBeUndefined();
        expect(getAgentConfig(name)?.thinking).toBeUndefined();
      }
    });
