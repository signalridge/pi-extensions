...
describe("buildAgentPrompt", () => {
  ...
  it("uses the original Git root for worktree isolation guidance", () => {
    const config = getDefaultConfig("general-purpose");
    const prompt = buildAgentPrompt(config, "/tmp/worktree/packages/api", env, undefined, {
      worktreeBase: "/repo",
    });

    expect(prompt).toContain("writable isolated git worktree copy of /repo");
    expect(prompt).toContain("base checkout at /repo is off-limits");
    expect(prompt.indexOf("<worktree_isolation>")).toBeGreaterThan(prompt.indexOf("Working directory: /tmp/worktree/packages/api"));
  });
