...
describe("FleetList navigation", () => {
  ...
  it("also activates on left (matches the '← agents' hint)", () => {
    const h = harness([makeRecord()]);
  ...
  it("ignores key-release events so one tap moves exactly one row", () => {
    ...
    expect(h.render().find(l => l.includes("main"))).toContain("<accent>");
    h.press(DOWN);          // a real second tap → first agent
    h.press(DOWN_RELEASE);
    expect(h.render().find(l => l.includes("one"))).toContain("<accent>");
    expect(h.render().find(l => l.includes("two"))).not.toContain("<accent>");
  });
  ...
  it("moves selection down/up and clamps at the ends", () => {
    ...
    expect(h.render().find(l => l.includes("two"))).toContain("<accent>");
    expect(h.render().find(l => l.includes("one"))).not.toContain("<accent>");
  });
  ...
  it("↑ above 'main' deactivates (returns to the prompt)", () => {
    ...
    expect(h.render().some(l => l.includes("← agents"))).toBe(true);
  });
  ...
  it("re-arms the refresh timer when the list is re-shown (toggle off→on)", () => {
    ...
    try {
      ...
      const manager = { listAgentsMutable: listAgents, abort: () => true } as unknown as AgentManager;
      const fleet = new FleetList(manager, new Map());
      fleet.setUICtx({
...
describe("FleetList rendering", () => {
  it("renders main + agent rows with markers, type, description and right-aligned stats", () => {
    ...
    expect(lines.find(l => l.includes("main"))).toContain("<accent>"); // main selected by default
    const agentLine = lines.find(l => l.includes("Sleep then report 1"))!;
    expect(agentLine).not.toContain("<accent>");
    expect(agentLine).toContain(getDisplayName("general-purpose"));
    expect(agentLine).toContain("13.1k tokens");
    expect(agentLine).toMatch(/\d+s ·/); // timing-agnostic right-aligned stats
    // Every row leads with its status mark, main included, so names line up.
    expect(agentLine).toContain("<dim>●</dim>");
    expect(lines.find(l => l.includes("main"))).toContain("<accent>●</accent>");
  });

  it("renders a steered run as wrapped up with warning styling", () => {
    const record = makeRecord({ status: "steered", completedAt: Date.now(), description: "partial work" });
    const output = harness([record]).render(200).join("\n");
    expect(output).toContain("<warning>wrapped up · turn limit</warning>");
    expect(output).not.toContain("completed · turn limit");
  });
  ...
  it("collapses overflow into a text more-below indicator", () => {
    const agents = Array.from({ length: 8 }, (_, i) =>
      makeRecord({ id: `a${i}`, description: `report ${i}` }));
    const h = harness(agents);
    const lines = h.render(120);
    // 8 agents, cap 5 visible -> "3 more below"
    expect(lines.some(l => l.includes("3 more below"))).toBe(true);
  });
  ...
  it("windows the visible agents so the selection stays on screen", () => {
    ...
    const lines = h.render(120);
    expect(lines.find(l => l.includes("report 7"))).toContain("<accent>");
    expect(lines.some(l => l.includes("more above"))).toBe(true); // hidden-above indicator
  });
});
...
describe("FleetList overlay lifecycle", () => {
  ...
  it("keeps the cursor on the viewed agent after closing, even if the list reordered", async () => {
    ...
    expect(h.render().find(l => l.includes("two"))).toContain("<accent>");
    expect(h.render().find(l => l.includes("three"))).not.toContain("<accent>");
  });
