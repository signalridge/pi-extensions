...
/**
 * Wait for a predicate with a load-tolerant deadline. The package Vitest
 * config serializes this real-timer file; the longer bound is still finite and
 * turns a leaked timer or missed callback into a deterministic failure.
 */
...
describe("SubagentScheduler — end-to-end with real timers", () => {
  ...
  it("interval job: fires repeatedly, runCount grows", async () => {
    ...
    await waitFor(() => manager.spawn.mock.calls.length >= 3, 5_000);
