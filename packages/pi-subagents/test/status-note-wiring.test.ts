...
describe("status note reaches the parent through the real handlers", () => {
  ...
  it("hides nested records from top-level tools, registry, transcripts, and lifecycle", async () => {
    let finishTop!: (value: unknown) => void;
    vi.mocked(runAgent).mockResolvedValue({
    ...
    vi.mocked(runAgent).mockImplementationOnce(() => new Promise((resolve) => { finishTop = resolve; }));
    const { pi, tools, lifecycle } = makePi();
    subagentsExtension(pi);
    await bind(lifecycle);
    ...
    finishTop({
      responseText: "top result",
      session: { dispose: vi.fn() } as any,
      aborted: false,
      steered: false,
    });
    await rawManager.getRecordMutable(topId)!.promise;
    await new Promise(resolve => setTimeout(resolve, 0));
  ...
  it("legacy RPC stop cannot abort a nested owner or its hidden grandchild", async () => {
    const resolvers: Array<(value: unknown) => void> = [];
    vi.mocked(runAgent).mockImplementation(() => new Promise((resolve) => { resolvers.push(resolve); }));
    const { pi, tools, eventHandlers, lifecycle } = makePi();
    subagentsExtension(pi);
    await bind(lifecycle);

    const root = await tools.get("Agent").execute(
      "tc-rpc-root",
      { prompt: "root", description: "root", subagent_type: "general-purpose", run_in_background: true },
      undefined, undefined, ctx(),
    );
    const rootId = textOf(root).match(/Agent ID: (\S+)/)?.[1];
    expect(rootId).toBeTruthy();
    const manager = vi.mocked(runAgent).mock.lastCall![3].nestedRuntime.manager;
    const childId = manager.spawn(pi, ctx(), "general-purpose", "child", {
      description: "child", isBackground: true, parentAgentId: rootId,
    });
    const grandchildId = manager.spawn(pi, ctx(), "general-purpose", "grandchild", {
      description: "grandchild", isBackground: true, parentAgentId: childId,
    });

    await eventHandlers.get("subagents:rpc:stop")?.({ requestId: "legacy-nested-stop", agentId: childId });
    expect(manager.getRecordMutable(childId)?.status).toBe("running");
    expect(manager.getRecordMutable(grandchildId)?.status).toBe("running");
    expect(pi.events.emit).toHaveBeenCalledWith(
      "subagents:rpc:stop:reply:legacy-nested-stop",
      { success: false, error: "Agent not found" },
    );

    for (const resolve of resolvers) {
      resolve({ responseText: "", session: { dispose: vi.fn() }, aborted: false, steered: false });
    }
    await Promise.all([rootId, childId, grandchildId].map((id) => manager.getRecordMutable(id)?.promise));
  });
