...
describe("agent-runner final output capture", () => {
  ...
  it("fails before creating a session when the caller signal is already aborted", async () => {
    const controller = new AbortController();
    controller.abort(new Error("caller cancelled"));

    await expect(runAgent(ctx, "Explore", "should not run", { pi, signal: controller.signal }))
      .rejects.toThrow("caller cancelled");
    expect(createAgentSession).not.toHaveBeenCalled();
    expect(defaultResourceLoaderCtor).not.toHaveBeenCalled();
  });

  it("forwards an abort during extension binding and does not prompt", async () => {
    const { session } = createSession("should not run");
    createAgentSession.mockResolvedValue({ session });
    const controller = new AbortController();
    session.bindExtensions.mockImplementation(async () => controller.abort(new Error("binding cancelled")));

    await expect(runAgent(ctx, "Explore", "should not run", { pi, signal: controller.signal }))
      .rejects.toThrow("binding cancelled");
  ...
  it("does not lose an abort between the signal check and listener installation", async () => {
    const { session } = createSession("should not run");
    createAgentSession.mockResolvedValue({ session });
    const controller = new AbortController();
    vi.spyOn(controller.signal, "addEventListener").mockImplementation(() => {
      controller.abort(new Error("check/add race"));
    });

    await expect(runAgent(ctx, "Explore", "should not run", { pi, signal: controller.signal }))
      .rejects.toThrow("check/add race");
    expect(session.abort).toHaveBeenCalledTimes(1);
    expect(session.dispose).toHaveBeenCalledTimes(1);
    expect(session.prompt).not.toHaveBeenCalled();
  });
...
describe("agent-runner session persistence", () => {
  ...
  it("passes the parent session file when persistent child sessions are enabled", async () => {
    vi.mocked(getAgentConfig).mockReturnValueOnce(makeAgentConfig({ persistSession: true }));
    settingsManagerGetSessionDir.mockReturnValue("/normal/pi/sessions");
    const { session } = createSession("OK");
    createAgentSession.mockResolvedValue({ session });
    const childCtx = {
      ...
      sessionManager: {
        ...ctx.sessionManager,
        getSessionFile: vi.fn(() => "/parent/sessions/root.jsonl"),
      },
    };

    await runAgent(childCtx, "Explore", "go", { pi });

    expect(sessionManagerCreate).toHaveBeenCalledWith(
      "/tmp",
      "/normal/pi/sessions",
      { parentSession: "/parent/sessions/root.jsonl" },
    );
  });
...
describe("agent-runner master tool allowlist", () => {
  it("the internal generation policy exposes no built-in, extension, or nested tools", async () => {
    vi.mocked(getConfig).mockReturnValueOnce(makeConfig({ extensions: true }));
    vi.mocked(getAgentConfig).mockReturnValueOnce(
      makeAgentConfig({ extensions: true, allowedSubagents: "all", memory: "project" }),
    );
    withExtensions({ "/ext/rogue.ts": ["rogue_tool"] });
    const { session } = createSession("OK");
    createAgentSession.mockResolvedValue({ session });

    await runAgent(ctx, "Explore", "draft", {
      pi,
      nestedRuntime: { manager: {} as any, parentAgentId: "parent", depth: 1 },
      [INTERNAL_AGENT_CONFIG_OVERRIDE]: AGENT_DEFINITION_GENERATION_OVERRIDE,
    });
    ...
    expect(lastToolsPassed()).not.toEqual(expect.arrayContaining(["bash", "write"]));
...
describe("extensionCanonicalNames (#143 — package short name alias)", () => {
  ...
  it("aliases a package-declared index.ts entry to the unscoped, lowercased package name", () => {
    ...
    const dir = pkgDir("@signalridge/Pi-Subagents", ["./src/index.ts"]);
  ...
  it("matches a package-installed extension by its package short name, not just its src dir (#143)", async () => {
    ...
    try {
      writeFileSync(
        ...
        JSON.stringify({ name: "@signalridge/pi-subagents", pi: { extensions: ["./src/index.ts"] } }),
      );
      mkdirSync(join(dir, "src"));
      writeFileSync(join(dir, "src", "index.ts"), "export default () => {};");
