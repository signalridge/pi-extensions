...
import { statusText } from "../src/ui/schedule-menu.js";

function makeMockManager() {
  const spawnFn = vi.fn(() => "agent-" + Math.random().toString(36).slice(2, 10));
  const getRecord = vi.fn(() => ({ status: "completed", promise: Promise.resolve("done") }));
  return {
    spawn: spawnFn,
    getRecord,
    getRecordMutable: getRecord,
  } as any;
}
...
describe("schedule menu status", () => {
  ...
  it("shows running and error before disabled", () => {
    expect(statusText({ ...base, enabled: false, lastStatus: "running" })).toBe("running");
    expect(statusText({ ...base, enabled: false, lastStatus: "error" })).toBe("error");
  });

  it("shows disabled only when no active outcome masks it", () => {
    expect(statusText({ ...base, enabled: false })).toBe("disabled");
    expect(statusText({ ...base, enabled: true })).toBe("ready");
  });
});
